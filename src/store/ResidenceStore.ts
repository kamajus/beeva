  ...
  return {
    ...
    pushResidence: (residence, user) => {
      set((state) => {
        ...
        const residenceIndex = residences.findIndex(({ residence: r }) => r.id === residence.id);
        if (residenceIndex > -1) {
    ...
    removeResidence: (id: string) => {
      set((state) => ({
        ...
        favoritesResidences: state.favoritesResidences.filter((r) => r.id !== id),
        cachedResidences: state.cachedResidences.filter(({ residence: r }) => r.id !== id),
      }));
    },
    resetResidenceCache: () => {
      set((state) => ({
